...
- (void)ap_forwardDraggingDestinationTo:(id<NSDraggingDestination>)draggingDestination {
    ...
    [self.class ap_swizzleToForwardDraggingDestinationIfNeeded:NSStringFromClass([draggingDestination class])];
    className = NSStringFromClass([draggingDestination class]);
}

+ (void)ap_swizzleToForwardDraggingDestinationIfNeeded:(NSString *)nameClass {
    if(![className isEqualToString:nameClass]){
